...
** gptel
   ...
   #+begin_src emacs-lisp
     (use-package gptel
       ...
       (add-to-list 'gptel-directives '(py-doc . "You are a experienced python programmer specialized on extending a python function or class with short and precise docstrings and annotations. You do not change the code in any way! If you see docstrings that do not follow the Google Style you convert then into the Google Style. Preserve the function with its comments. ONLY add docstrings and annotations if necessary. Also no additional explanations from your side or any ```python marks, this would be a desasters ruining by code!"))
       (add-to-list 'gptel-directives '(py-complete . "You are an experienced python programmer. You make short proposals for how to continue an implementation or if you see instructions introduced with `# AI: ` follow those instructions. That means you DO NOT REPEAT the existing code but only provide the code that should follow. It is forbidden to use ```python tags because this will ruin the usage of your code."))
